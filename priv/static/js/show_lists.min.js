...
window.showRemoveAlbum=!0;toggleRemoveAlbumButtons();window.addAlbumModal.close()}else M.toast({html:c.responseText.replace(/"/g,"")})};c.open("PATCH","/lists/"+b);c.setRequestHeader("Content-Type","application/json");c.setRequestHeader("X-CSRF-Token",CSRF_TOKEN);c.send(JSON.stringify({action:"add_album",currentAlbum:a}))},toggleRemoveAlbumButtons=function(){window.showRemoveAlbum?(window.showRemoveAlbum=!1,hideClass("delete-button")):(window.showRemoveAlbum=!0,showClass("delete-button"),M.toast({html:"\n        <span>\n          Click the <i class='small material-icons' style='vertical-align:middle;'>highlight_off</i> to remove an album from the list\n        </span>\n      ",
displayLength:2500}))};document.getElementById("remove-album").addEventListener("click",toggleRemoveAlbumButtons);window.showRemoveAlbum=!1;window.removeAlbumFromList=function(a){if(confirm("Are you sure you want to remove the album from this list? You cannot undo this operation.")){var b=parseInt(window.location.pathname.replace("/lists/","")),c=new XMLHttpRequest;c.onreadystatechange=function(){if(4===c.readyState)if(200<=c.status&&300>c.status){var b=listVueApp.albums.findIndex(function(b){return b.id==
a});listVueApp.albums.splice(b,1);M.toast({html:c.responseText.replace(/"/g,"")})}else M.toast({html:"Unable to remove album from list"})};c.open("PATCH","/lists/"+b);c.setRequestHeader("Content-Type","application/json");c.setRequestHeader("X-CSRF-Token",CSRF_TOKEN);c.send(JSON.stringify({action:"remove_album",albumID:a}))}};document.getElementById("list-name-input").addEventListener("keyup",function(a){13===a.keyCode&&updateListTitle()});var albumSearchSpinner=document.getElementById("add-album-search-spinner-container");
document.getElementById("add-album-input").addEventListener("keydown",function(a){if(13==a.keyCode){removeSelectedElement(".album-search-album");document.getElementById("add-album-search-results").style.display="inline-flex";removeSelectedElement("#add-album-search-result-warning");showAlbumSearchSpinner();a=document.getElementById("add-album-input").value.trim();var b=new XMLHttpRequest;b.onreadystatechange=function(){4===b.readyState&&(200<=b.status&&300>b.status?displayAlbumSearchResults(JSON.parse(b.responseText)):
(hideAlbumSearchSpinner(),console.log("error",b)))};b.open("GET","/api/apple/search/?search_string="+a+"&offset=0");b.send()}})}function selectChip(a){a.classList.add("light-blue");a.classList.add("accent-4");a.classList.add("white-text")}function deselectChip(a){a.classList.remove("light-blue");a.classList.remove("accent-4");a.classList.remove("white-text")}function toggleChipSelect(a){a.classList.toggle("light-blue");a.classList.toggle("accent-4");a.classList.toggle("white-text")}
