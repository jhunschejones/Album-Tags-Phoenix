...
-1==c?(listVueApp.artistFilters.push(a.target.dataset.artist),document.getElementById("artist-filter-input")&&(a=document.getElementById("artist-filter-input").value.trim(),a=listVueApp.artistFilters.indexOf(a),-1!=a&&listVueApp.artistFilters.splice(a,1))):listVueApp.artistFilters.splice(c,1);listPage.clearArtistFilterInput()},selectTagFilter:function(a){listPage.toggleChipSelect(a.target);listPage.clearTagFilterInput();var c=listVueApp.tagFilters.indexOf(a.target.dataset.tag);-1==c?listVueApp.tagFilters.push(a.target.dataset.tag):
listVueApp.tagFilters.splice(c,1);listVueApp.resetSelectedAlbums();listPage.filterAll();setTimeout(function(){listPage.hilightSelectedTagFilters()},10)},removeFilter:function(a){var c=a.target.dataset.filter;switch(a.target.dataset.type){case "year":a=listVueApp.yearFilters.indexOf(c);listVueApp.yearFilters.splice(a,1);break;case "artist":a=listVueApp.artistFilters.indexOf(c);listVueApp.artistFilters.splice(a,1);break;case "tag":a=listVueApp.tagFilters.indexOf(c),listVueApp.tagFilters.splice(a,1)}listVueApp.resetSelectedAlbums();
...
document.getElementsByClassName("year-filter"),c=listVueApp.yearFilters,b=0;b<a.length;b++){var d=a[b];c.includes(d.dataset.year)?listPage.selectChip(d):listPage.deselectChip(d)}},filterYears:function(){0!==listVueApp.yearFilters.length&&(listVueApp.selectedAlbums=listVueApp.selectedAlbums.filter(function(a){return listVueApp.yearFilters.includes(a.release_date.substr(0,4))}))},filterArtists:function(){0!==listVueApp.artistFilters.length&&(listVueApp.selectedAlbums=listVueApp.selectedAlbums.filter(function(a){return 0<
listVueApp.artistFilters.filter(function(c){return a.artist.toUpperCase().includes(c.toUpperCase())}).length}))},filterTags:function(){0!==listVueApp.tagFilters.length&&(listVueApp.selectedAlbums=listVueApp.selectedAlbums.filter(function(a){return listVueApp.tagFilters.every(function(c){return 0<a.tags.map(function(a){return a.text}).filter(function(a){return a.toUpperCase().includes(c.toUpperCase())}).length})}))},clearSpecificFilter:function(a){switch(a.target.dataset.type){case "year":listVueApp.yearFilters=
...
g=document.getElementById("tag-filter-btn");var h=document.getElementById("tag-filter-input"),l=document.getElementById("toggle-filter-tag-input-btn"),k=document.getElementById("artist-filter-input"),m=document.getElementById("toggle-filter-artist-input-btn");listPage.showRemoveAlbum=!1;listPage.initializeMainFloatingActionButton();listPage.initializeYearFilterModal();listPage.initializeArtistFitlerModal();listPage.initializeTagFilterModal();a.addEventListener("click",listPage.showFilterMenu);c.addEventListener("click",
listPage.closeFilterMenu);b.addEventListener("click",listPage.clearAllFilters);d.addEventListener("click",listPage.closeFilterMenu);l.addEventListener("click",function(){h.classList.toggle("hide");listPage.clearTagFilterInput()});h.addEventListener("keyup",function(a){a=a.target.value.trim().replace(/\b\w+/g,function(a){return a.charAt(0).toUpperCase()+a.substr(1).toLowerCase()});listVueApp.tagFilters=0<a.length?[a]:[];listVueApp.resetSelectedAlbums();listPage.filterAll();setTimeout(function(){listPage.hilightSelectedTagFilters()},
100)});m.addEventListener("click",function(){k.classList.toggle("hide");listPage.clearArtistFilterInput()});k.addEventListener("keyup",function(a){a=a.target.value.trim();listVueApp.artistFilters=0<a.length?[a]:[];listVueApp.resetSelectedAlbums();listPage.filterAll();setTimeout(function(){listPage.hilightSelectedArtistFilters()},100)});e.addEventListener("click",function(){listPage.hilightSelectedYearFilters();listPage.yearFilterModal.open()});f.addEventListener("click",function(){listPage.hilightSelectedArtistFilters();
listPage.artistFilterModal.open()});g.addEventListener("click",function(){listPage.hilightSelectedTagFilters();listPage.tagFilterModal.open()});document.addEventListener("click",function(a){listPage.closeToastsOnClick(a)});document.addEventListener("click",function(a){a.target.parentNode&&listPage.closeFloatingActionButtonWhenClickAway(a)},{passive:!0});document.addEventListener("scroll",function(){setTimeout(function(){listPage.closeFilterMenu()},50)},{passive:!0});document.addEventListener("click",
function(a){listPage.closeModalOnClickAway(a)},{passive:!0});addEventListenerToClass("clear-filters","click",function(a){listPage.clearSpecificFilter(a)});addEventListenerToClass("filter-btn","click",function(a){listPage.yearFilterModal.close();listPage.artistFilterModal.close();listPage.tagFilterModal.close()})});function showClass(a){a=document.getElementsByClassName(a);for(var c=0;c<a.length;c++)a[c].classList.remove("hide")}
function hideClass(a){a=document.getElementsByClassName(a);for(var c=0;c<a.length;c++)a[c].classList.add("hide")}function addEventListenerToClass(a,c,b){a=document.getElementsByClassName(a);for(var d=0;d<a.length;d++)a[d].addEventListener(c,b)};
