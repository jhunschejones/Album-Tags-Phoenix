...
document.addEventListener("DOMContentLoaded",function(){var a=document.querySelectorAll(".fixed-action-btn"),c=M.FloatingActionButton.init(a,{direction:"top",hoverEnabled:!1});document.addEventListener("click",function(a){if(!["lists-fab-button","add-lists","edit-lists","remove-album"].includes(a.target.parentNode.id))for(a=0;a<c.length;a++)c[a].close()})});document.addEventListener("click",function(a){a=a.target.parentNode;(a.classList&&a.classList.contains("toast")||"toast-container"==a.id)&&closeToast()});
function closeToast(){var a=document.querySelector(".toast");M.Toast.getInstance(a).dismiss()}
document.addEventListener("DOMContentLoaded",function(){var a=document.getElementById("edit-list-modal");window.editListModal=M.Modal.init(a,{onOpenEnd:function(){var a=document.getElementById("list-name-input");a.focus();a.selectionStart=a.selectionEnd=a.value.length}});document.getElementById("edit-list").addEventListener("click",function(){window.editListModal.open()});document.getElementById("clear-list-name-input-btn").addEventListener("click",function(a){a=document.getElementById("list-name-input");
a.value="";a.focus()});a=document.getElementById("add-album-modal");window.addAlbumModal=M.Modal.init(a,{onOpenEnd:function(){document.getElementById("add-album-input").focus()}});document.getElementById("add-album").addEventListener("click",function(){window.addAlbumModal.open()});document.getElementById("clear-add-album-input-btn").addEventListener("click",function(a){a=document.getElementById("add-album-input");a.value="";a.focus()})});
function showClass(a){a=document.getElementsByClassName(a);for(var c=0;c<a.length;c++)a[c].classList.remove("hide")}function hideClass(a){a=document.getElementsByClassName(a);for(var c=0;c<a.length;c++)a[c].classList.add("hide")}
function removeSelectedElement(a){if("."===a[0]){a=document.querySelectorAll(a);for(var c=0;c<a.length;c++)a[c].parentNode.removeChild(a[c])}else"#"===a[0]?(a=document.getElementById(a.substring(1)))&&a.parentNode.removeChild(a):console.error("removeSelectedElement() was passed an invalid selector")}function stringToNode(a){var c=document.createElement("template");c.innerHTML=a;return c.content.firstChild}window.showRemoveAlbum=!1;
function toggleRemoveAlbumButtons(){window.showRemoveAlbum?(window.showRemoveAlbum=!1,hideClass("delete-button")):(window.showRemoveAlbum=!0,showClass("delete-button"))}document.getElementById("remove-album").addEventListener("click",toggleRemoveAlbumButtons);
