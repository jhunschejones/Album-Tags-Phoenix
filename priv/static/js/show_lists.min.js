...
listPage.filterAll()}})},initializeArtistFitlerModal:function(){var a=document.getElementById("artist-filter-modal");listPage.artistFilterModal=M.Modal.init(a,{onCloseStart:function(){listVueApp.resetSelectedAlbums();listPage.filterAll()}})},initializeTagFilterModal:function(){var a=document.getElementById("tag-filter-modal");listPage.tagFilterModal=M.Modal.init(a,{onCloseStart:function(){listVueApp.resetSelectedAlbums();listPage.filterAll()}})},toggleRemoveAlbumButtons:function(){listPage.showRemoveAlbum?
(listPage.showRemoveAlbum=!1,hideClass("delete-button")):(listPage.showRemoveAlbum=!0,showClass("delete-button"),M.toast({html:"\n        <span>\n          Click the <i class='small material-icons' style='vertical-align:middle;'>highlight_off</i> to remove an album from the list\n        </span>\n      ",displayLength:2500}))},addAlbumToList:function(a){var c=parseInt(window.location.pathname.replace("/lists/","")),b=new XMLHttpRequest;b.onreadystatechange=function(){if(4===b.readyState)if(200<=b.status&&
300>b.status){var a=JSON.parse(b.responseText);listVueApp.albums.push(a.added_album);listPage.showRemoveAlbum=!0;listPage.toggleRemoveAlbumButtons();setTimeout(function(){listVueApp.resetSelectedAlbums();listPage.filterAll()},5);listPage.addAlbumModal.close()}else M.toast({html:b.responseText.replace(/"/g,"")})};b.open("PATCH","/lists/"+c);b.setRequestHeader("Content-Type","application/json");b.setRequestHeader("X-CSRF-Token",CSRF_TOKEN);b.send(JSON.stringify({action:"add_album",currentAlbum:a}))},
removeAlbumFromList:function(a){if(confirm("Are you sure you want to remove the album from this list? You cannot undo this operation.")){var c=parseInt(window.location.pathname.replace("/lists/","")),b=new XMLHttpRequest;b.onreadystatechange=function(){if(4===b.readyState)if(200<=b.status&&300>b.status){var c=listVueApp.albums.findIndex(function(b){return b.id==a});listVueApp.albums.splice(c,1);setTimeout(function(){listVueApp.resetSelectedAlbums();listPage.filterAll();0==listVueApp.selectedAlbums.length&&
(listVueApp.yearFilters=[],listVueApp.artistFilters=[],listVueApp.tagFilters=[],listVueApp.resetSelectedAlbums(),listPage.filterAll())},5);M.toast({html:b.responseText.replace(/"/g,"")})}else M.toast({html:"Unable to remove album from list"})};b.open("PATCH","/lists/"+c);b.setRequestHeader("Content-Type","application/json");b.setRequestHeader("X-CSRF-Token",CSRF_TOKEN);b.send(JSON.stringify({action:"remove_album",albumID:a}))}},updateListTitle:function(){var a=parseInt(window.location.pathname.replace("/lists/",
"")),c=document.getElementById("list-name-input").value.trim(),b=new XMLHttpRequest;b.onreadystatechange=function(){if(4===b.readyState)if(200<=b.status&&300>b.status){var a=JSON.parse(b.responseText);listVueApp.listTitle=a.list_title;listPage.editListModal.close();M.toast({html:a.message})}else M.toast({html:b.responseText.replace(/"/g,"")})};b.open("PATCH","/lists/"+a);b.setRequestHeader("Content-Type","application/json");b.setRequestHeader("X-CSRF-Token",CSRF_TOKEN);b.send(JSON.stringify({action:"update_title",
newTitle:c}))},searchForAlbumsToAddToList:function(){removeSelectedElement(".album-search-album");document.getElementById("add-album-search-results").style.display="inline-flex";removeSelectedElement("#add-album-search-result-warning");listPage.showAlbumSearchSpinner();var a=document.getElementById("add-album-input").value.trim(),c=new XMLHttpRequest;c.onreadystatechange=function(){4===c.readyState&&(200<=c.status&&300>c.status?listPage.displayAlbumSearchResults(JSON.parse(c.responseText)):(listPage.hideAlbumSearchSpinner(),
console.log("error",c)))};c.open("GET","/api/apple/search/?search_string="+a+"&offset=0");c.send()},showAlbumSearchSpinner:function(){listPage.albumSearchSpinner.classList.remove("hide")},hideAlbumSearchSpinner:function(){listPage.albumSearchSpinner.classList.contains("hide")||listPage.albumSearchSpinner.classList.add("hide")},displayAlbumSearchResults:function(a){var c=document.getElementById("add-album-search-results");1>a.albums.length&&(listPage.hideAlbumSearchSpinner(),c.appendChild(stringToNode('<div id="add-album-search-result-warning" class="center-align">\n          <em class="grey-text text-darken-1">\n            No albums match your search! Try another artist or album.\n          </em>\n        </div>')),
document.getElementById("add-album-search-results").style.display="block");listPage.hideAlbumSearchSpinner();document.getElementById("add-album-search-results").style.display="inline-flex";for(var b=0;b<a.albums.length;b++){var d=a.albums[b],e=d.cover.replace("{w}","230").replace("{h}","230");c.appendChild(stringToNode('<div class="album-search-album" onclick="listPage.addAlbumToList('+d.appleAlbumID+')">\n          <img class="search-album-cover" src="'+e+'">\n          <p class="search-title">'+
d.title+'</p>\n          <p class="search-artist grey-text text-darken-1">'+d.artist+"</p>\n        </div>"))}},selectChip:function(a){a.classList.add("light-blue");a.classList.add("accent-4");a.classList.add("white-text")},deselectChip:function(a){a.classList.remove("light-blue");a.classList.remove("accent-4");a.classList.remove("white-text")},toggleChipSelect:function(a){a.classList.toggle("light-blue");a.classList.toggle("accent-4");a.classList.toggle("white-text")},selectYearFilter:function(a){listPage.toggleChipSelect(a.target);
var c=listVueApp.yearFilters.indexOf(a.target.dataset.year);-1==c?listVueApp.yearFilters.push(a.target.dataset.year):listVueApp.yearFilters.splice(c,1)},selectArtistFilter:function(a){listPage.toggleChipSelect(a.target);var c=listVueApp.artistFilters.indexOf(a.target.dataset.artist);-1==c?listVueApp.artistFilters.push(a.target.dataset.artist):listVueApp.artistFilters.splice(c,1)},selectTagFilter:function(a){listPage.toggleChipSelect(a.target);var c=listVueApp.tagFilters.indexOf(a.target.dataset.tag);
-1==c?listVueApp.tagFilters.push(a.target.dataset.tag):listVueApp.tagFilters.splice(c,1);listVueApp.resetSelectedAlbums();listPage.filterAll();setTimeout(function(){listPage.hilightSelectedTagFilters()},10)},removeFilter:function(a){var c=a.target.dataset.filter;switch(a.target.dataset.type){case "year":a=listVueApp.yearFilters.indexOf(c);listVueApp.yearFilters.splice(a,1);break;case "artist":a=listVueApp.artistFilters.indexOf(c);listVueApp.artistFilters.splice(a,1);break;case "tag":a=listVueApp.tagFilters.indexOf(c),
listVueApp.tagFilters.splice(a,1)}listVueApp.resetSelectedAlbums();listPage.filterAll()},hilightSelectedTagFilters:function(){for(var a=document.getElementsByClassName("tag-filter"),c=listVueApp.tagFilters,b=0;b<a.length;b++){var d=a[b];c.includes(d.dataset.tag)?listPage.selectChip(d):listPage.deselectChip(d)}},hilightSelectedArtistFilters:function(){for(var a=document.getElementsByClassName("artist-filter"),c=listVueApp.artistFilters,b=0;b<a.length;b++){var d=a[b];c.includes(d.dataset.artist)?listPage.selectChip(d):
listPage.deselectChip(d)}},hilightSelectedYearFilters:function(){for(var a=document.getElementsByClassName("year-filter"),c=listVueApp.yearFilters,b=0;b<a.length;b++){var d=a[b];c.includes(d.dataset.year)?listPage.selectChip(d):listPage.deselectChip(d)}},filterYears:function(){0!==listVueApp.yearFilters.length&&(listVueApp.selectedAlbums=listVueApp.selectedAlbums.filter(function(a){return listVueApp.yearFilters.includes(a.release_date.substr(0,4))}))},filterArtists:function(){0!==listVueApp.artistFilters.length&&
(listVueApp.selectedAlbums=listVueApp.selectedAlbums.filter(function(a){return listVueApp.artistFilters.includes(a.artist)}))},filterTags:function(){0!==listVueApp.tagFilters.length&&(listVueApp.selectedAlbums=listVueApp.selectedAlbums.filter(function(a){return listVueApp.tagFilters.every(function(c){return a.tags.map(function(a){return a.text}).includes(c)})}))},clearSpecificFilter:function(a){switch(a.target.dataset.type){case "year":listVueApp.yearFilters=[];M.toast({html:"Year filters cleared",
displayLength:2500});break;case "artist":listVueApp.artistFilters=[];M.toast({html:"Artist filters cleared",displayLength:2500});break;case "tag":listVueApp.tagFilters=[],M.toast({html:"Tag filters cleared",displayLength:2500})}listVueApp.resetSelectedAlbums();listPage.filterAll();listPage.hilightSelectedArtistFilters();listPage.hilightSelectedTagFilters();listPage.hilightSelectedYearFilters()},filterAll:function(){listPage.filterYears();listPage.filterArtists();listPage.filterTags()},setURIparams:function(a,
c){var b=new URL(document.location);if(0===c.length)return b.searchParams.delete(a),history.replaceState({},"",b);c=encodeURIComponent(c.join(",,"));b.searchParams.set(a,c);history.replaceState({},"",b)},getURIparam:function(a){return(a=(new URL(document.location)).searchParams.get(a))?decodeURIComponent(a).split(",,"):[]}},listVueApp=new Vue({el:"#listVueApp",data:{albums:allAlbums,selectedAlbums:[],listUserID:listUserID,yearFilters:[],artistFilters:[],tagFilters:[],listTitle:listTitle,titleTags:tagSearchTags},
methods:{albumCover:function(a,c){return a.cover.replace("{w}",c).replace("{h}",c)},removeAlbum:function(a){listPage.removeAlbumFromList(a)},resetSelectedAlbums:function(){var a=this,c=JSON.parse(JSON.stringify(this.albums));if(listUserID)for(var b=0;b<c.length;b++){var d=c[b];d.tags=d.tags.filter(function(b){return b.user_id==a.listUserID})}this.selectedAlbums=c.sort(function(a,b){return a.release_date<b.release_date?1:a.release_date>b.release_date?-1:0})}},computed:{selectedAlbumsCount:function(){return this.selectedAlbums.length},
artists:function(){return Array.from(new Set(this.selectedAlbums.slice().map(function(a){return a.artist}).sort(function(a,c){a=a.toUpperCase();c=c.toUpperCase();return a<c?-1:a>c?1:0})))},years:function(){return Array.from(new Set(this.selectedAlbums.slice().map(function(a){return a.release_date.substr(0,4)})))},tags:function(){return Array.from(new Set(this.selectedAlbums.slice().map(function(a){return a.tags.map(function(a){return a.text})}).flat().sort()))},allFilters:function(){return this.yearFilters.concat(this.artistFilters).concat(this.tagFilters)}},
watch:{albums:function(){this.resetSelectedAlbums()},yearFilters:function(a){listPage.setURIparams("years",a)},tagFilters:function(a){listPage.setURIparams("tags",a)},artistFilters:function(a){listPage.setURIparams("artists",a)}},beforeMount:function(){this.resetSelectedAlbums()},mounted:function(){var a=document.getElementById("htm-title");a.parentNode.removeChild(a);this.artistFilters=listPage.getURIparam("artists");this.yearFilters=listPage.getURIparam("years");this.tagFilters=listPage.getURIparam("tags");
