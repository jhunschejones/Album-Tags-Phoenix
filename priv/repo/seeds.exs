...
alias AlbumTags.{Accounts, Albums, Lists, Repo}

# ====== create test users ======
if !Repo.get_by(Accounts.User, email: "carl@dafox.com") do
...
carl = Repo.get_by!(Accounts.User, email: "carl@dafox.com")

if !Repo.get_by(Accounts.User, email: "daisy@dafox.com") do
  Accounts.create_user(
    %{
      email: "daisy@dafox.com",
      provider: "google",
      token: "c7be72feb30608cf810cc9bce2d856dc839a11aa"
    }
  )
end

daisy = Repo.get_by!(Accounts.User, email: "daisy@dafox.com")
...
artifex_pereo = Repo.get_by!(Albums.Album, apple_album_id: 1135092935)
...
# ====== create test tags ======
if !Repo.get_by(Albums.Tag, [text: "Emo", user_id: daisy.id]) do
  Albums.create_tag(%{text: "Emo", user_id: daisy.id, custom_genre: true})
end
emo_tag = Repo.get_by!(Albums.Tag, [text: "Emo", user_id: daisy.id])

if !Repo.get_by(Albums.Tag, [text: "Screamo", user_id: carl.id]) do
  Albums.create_tag(%{text: "Screamo", user_id: carl.id, custom_genre: true})
end
screamo_tag = Repo.get_by!(Albums.Tag, [text: "Screamo", user_id: carl.id])

if !Repo.get_by(Albums.Tag, [text: "Noodley Guitars", user_id: carl.id]) do
  Albums.create_tag(%{text: "Noodley Guitars", user_id: carl.id, custom_genre: false})
end
guitars_tag = Repo.get_by!(Albums.Tag, [text: "Noodley Guitars", user_id: carl.id])

if !Repo.get_by(Albums.Tag, [text: "2005", user_id: carl.id]) do
  Albums.create_tag(%{text: "2005", user_id: carl.id, custom_genre: false})
end
tag_2005 = Repo.get_by!(Albums.Tag, [text: "2005", user_id: carl.id])

# ====== associate test tags with test albums ======

if !Repo.get_by(Albums.AlbumTag, [album_id: emery.id, tag_id: emo_tag.id, user_id: daisy.id]) do
  Albums.add_tag_to_album(%{album_id: emery.id, tag_id: emo_tag.id, user_id: daisy.id})
end

if !Repo.get_by(Albums.AlbumTag, [album_id: treos.id, tag_id: emo_tag.id, user_id: daisy.id]) do
  Albums.add_tag_to_album(%{album_id: treos.id, tag_id: emo_tag.id, user_id: daisy.id})
end

if !Repo.get_by(Albums.AlbumTag, [album_id: emery.id, tag_id: screamo_tag.id, user_id: carl.id]) do
  Albums.add_tag_to_album(%{album_id: emery.id, tag_id: screamo_tag.id, user_id: carl.id})
end

if !Repo.get_by(Albums.AlbumTag, [album_id: treos.id, tag_id: guitars_tag.id, user_id: carl.id]) do
  Albums.add_tag_to_album(%{album_id: treos.id, tag_id: guitars_tag.id, user_id: carl.id})
end

if !Repo.get_by(Albums.AlbumTag, [album_id: emery.id, tag_id: tag_2005.id, user_id: carl.id]) do
  Albums.add_tag_to_album(%{album_id: emery.id, tag_id: tag_2005.id, user_id: carl.id})
end

if !Repo.get_by(Albums.AlbumTag, [album_id: treos.id, tag_id: tag_2005.id, user_id: carl.id]) do
  Albums.add_tag_to_album(%{album_id: treos.id, tag_id: tag_2005.id, user_id: carl.id})
end

# ====== create test list ======
if !Repo.get_by(Lists.List, [title: "Test List", user_id: carl.id]) do
  Lists.create_list(%{permalink: Ecto.UUID.generate(), title: "Test List", user_id: carl.id, private: false})
end
seed_list = Repo.get_by!(Lists.List, [title: "Test List", user_id: carl.id])

# ====== associate test albums with test list ======
if !Repo.get_by(Lists.AlbumList, [album_id: emery.id, list_id: seed_list.id, user_id: carl.id]) do
  Lists.add_album_to_list(%{album_id: emery.id, list_id: seed_list.id, user_id: carl.id})
end

if !Repo.get_by(Lists.AlbumList, [album_id: treos.id, list_id: seed_list.id, user_id: carl.id]) do
  Lists.add_album_to_list(%{album_id: treos.id, list_id: seed_list.id, user_id: carl.id})
end

# ====== create test album connections ======
if !Repo.get_by(Albums.AlbumConnection, [parent_album: emery.id, child_album: treos.id, user_id: carl.id]) do
  Albums.create_album_connection(%{parent_album: emery.id, child_album: treos.id, user_id: carl.id})
end

if !Repo.get_by(Albums.AlbumConnection, [parent_album: treos.id, child_album: artifex_pereo.id, user_id: carl.id]) do
